...
#+LATEX: \begin{center}6. Mai 2022\end{center}
#+LATEX: \begin{center}{}\end{center}
...
Untersuchung mit Methoden aus dem Machine-Learning (ML) analysiert, wozu eigens ein
Script in der Programmiersprache Python angefertigt wurde. Weiters
wurden zusätzlich Applikationen von ML diskutiert und
...
Fakultätsseite sah, war das Praktikum für mich die erste Wahl.
...
persönlichen Austausch, da hier aufgrund der Covid-19-Pandemie
Einschränkungen zu erwarten waren. David Steyrl lernte ich auch kurz
zuvor in einer Vorlesung kennen, im Grunde ging es darum, ihn
kennenzulernen, sein Wissen und seine Erfahrung usw. Aufgrund der VU
im Vorsemester hatte ich schon ein ungefähres Bild.
...
herzustellen. Bisher kannte ich hier keine konkreten
Anwendungsfälle.

Mit dem Betreuer David Steyrl wurde daher vereinbart, einen bereits
mit klassischen statistischen Methoden (Korrelationen,
Moderationsanalyse) untersuchter Datensatz, der im Rahmen meiner
Bachelorarbeit erhoben wurde und Zusammenhänge zwischen
problematischem Alkoholkonsum und Mindfulness (sowie einer Reihe
anderer Prädiktoren), mit Machine-Learning-Methoden zu
untersuchen. Dabei sollten nicht die Ergebnisse der früheren und der
aktuellen Analyse gegenübergestellt werden, sondern ein Gefühl
entwickelt werden, wie eine solche ML-Analyse in der Praxis aussieht, da
mir bisher die Anwendungsfälle fehlten.

Weiteres Ziel war für mich eine Vertiefung des bisherigen Wissens über
zusätzliche Literaturrecherchen.
...
Online-Meeting mit jenen Masterstudenten statt, die David Steyrl im
Rahmen ihrer Masterarbeit betreut. Insgesamt schätze ich, dass die
aufgewendete Zeit zu 50% in Programmiertätigkeiten floss, etwa 10% in
die Kommunikation und etwa 40% in Literaturrecherche, Analyse und
Ergebnisse reflektieren.

* Analyse des Datensatzes
** Der Datensatz

Der verwendete Datensatz stammt aus meiner eigenen Bachelorarbeit und
wurde im Sommer 2021 über einen Online-Survey erhoben. 500
Versuchspersonen machten darin u.a. Angaben zu ihrem Alkoholkonsum,
Ausprägung von Impulsivität, Mindfulness, Bewältigungsmotive beim
Alkoholkonsum. Zusammen mit den soziodemografischen Variablen wurden
insgesamt 50 Variablen als Prädiktoren verwendet.

** Erstellung des Scripts

Für die Analyse der Daten war es notwendig, ein Script zu erstellen,
dass die folgenden Aufgaben erfüllt: (1) Daten einlesen, (2) Daten
bereinigen, indem auf die relevanten Variablen reduziert wird, diese
umbenannt und fehlende Werte imputiert werden (Median), (3)
...
optimalen Hyperparameter zu ermitteln, (7) Verfügung über ein Pool an Modellen
(ElasticNet, ExtraTrees und GradientBoostRegressor) und dazu passende
Hyperparameter-Räume, (8) jedes Modell einmal durchlaufen lassen,
(9) Aufzeichnen der R²-Scores, (10) Aufzeichnen der Shapely-Werte für die
Wichtigkeit der einzelnen Prädiktoren.
...
... bis zwei Code-Snippets des Scripts

#+end_example

** Ergebnisdarstellung

Bis das Script fehlerfreie Resultate lieferte, wurde das Script
dreimal komplett durchlaufen, wobei dieses jedes Mal etwa 12 Stunden
in Anspruch nahm. Die Resultate wurden anschließend für jeden Lauf in
einem eigenen Jupyter-Notebook (verfügbar im [[https://github.com/dominikpegler/internship_ml/blob/main/results.ipynb][Online-Repository]])
...
eingerichteten Online-Repositorium verfügbar gemacht
...
Im Zuge des Praktikums habe ich auch versucht, die Grundprinzipien
davon zu verstehen, wie künstliche neuronale Netzwerke lernen:
Aktivierung und Aktualisierung der Gewichte (Weights) und Biases. In
Abbilung ref:nn_OR ist die simpelste Form dargestellt: ein Neuron $y$
mit zwei Eingängen $x_0$ und $x_1$, von deren Aktivierungsmuster (in
diesem Fall die OR-Funktion) es abhängt, ob das Neuron feuert ($y =
1$) oder nicht ($y = 0$). Das Neuron wählt als Ausgangspunkt beim
Lernen des OR-Zusammenhangs erst zufällige Werte für beide Gewichte
$w_0$ und $w_1$) und den Bias ($b$). Über eine Aktivierungsfunktion
(hier die Sigmoid-Funktion) wird aus den Eingangswerten, den Gewichten
und dem Bias ein Wert berechnet, der entweder 0 oder 1 ergibt. Je
nachdem, wie weit das Neuron mit seiner Berechnung vom tatsächlichen
Wert abweicht ("Error"), passt es seine Gewichte und den Bias an. Über
mehrere Durchgänge ("Epochs") hinweg, wird die Schätzung des Neurons
somit immer genauer (Abb. ref:NN_learning_rate). Siehe hierzu die
Gradientenberechnung in der nachfolgenden Implementation mit Numpy:
...
** Beispiel: Training eines einzelnen Neurons mit Numpy
...
zur Verfügung, wenn ich Fragen hatte. Er hat mir im Erstgespräch und
im späteren Verlauf immer wieder klar gesagt, was er von mir erwartet,
und Feedback zu den von mir verrichteten Tätigkeiten gegeben. Sehr
wertvoll war für mich, dass ich nie das Gefühl hatte, eine für mich
wichtige Frage nicht stellen zu wollen. Bei meinen Fragen schien es
mir auch kein Thema auf dem Gebiet von Machine-Learning-Methoden
gegeben zu haben, zu dem er keine Erfahrungen hatte. Ich fand das
bereichernd, da es zwar irrsinnig viel Literatur zu diesen Themen
gibt, aber jemanden zu finden, der einem persönlich berichten kann,
ist nicht immer so einfach.

* MSc-Meetings
Wie schon kurz angesprochen, fand einmal wöchentlich ein
Online-Meeting via Skype mit den Masterstudenten statt, deren
...
insgesamt eine sehr interessante, aufschlussreich und wichtige
...
nämlich klassische Machine-Learning-Algorithmen, wie sie in
Scikit-Learn implementiert sind, die zweite Hälfte neuronale Netzwerke
mit Tensorflow.
* Programmieren mit Python
- Vorerfahrung
- nur Theorie zu ML musste gelernt bzw. aufgefrischt werden,
  Programmierug ging zügig
  
* Abschluss/Ausblick
- Was ich mitgenommen habe
- Wie sind Erwartungen erfüllt worden
  - an die Stelle
  - an mich selber
- Was als nächstes?
* Textstücke                                                       :noexport:
...
Zu allererst ging es für mich dabei darum, Vorkenntnisse aufzufrischen
und die Methode der Linearen Regression mit Python zu
wiederhohlen. Diese ist Ausgangspunkt jeden maschinellen Lernens. Im
nächsten Schritt wurden regularisierte lineare Regressionsmodelle und
deren Hyperparameter studiert: RidgeRegression, LassoRegression und
der Hybrid aus beiden: ElasticNet. Danach kommen Random Forests, Extra
Trees und der Gradien Boost Regressor. Decision Trees wurden
ausgelassen. Aufgrund der Fragestellung mit einer kontinuierlichen
Outcome-Variable wurden keine Klassifikationsalgorithmen in diesem
Praktikum verwendet.
...
Was ist rausgekommen bei meinen Untersuchungen?

- ElasticNet hat überraschend am besten abgeschnitten
- ExtraTrees und GB gleichauf
- Die Ergebnisse schildere ich dann noch als gscheiten Plot-Output
  hier.

** Diskussion

Wie geht es jetzt weiter nach dem Praktikum?

Was sind die Take-Away-Points? Was habe ich besonders gelernt?

Worauf baue ich jetzt auf?

Auf jeden Fall möchte ich das Script und damit die
Scikit-Learn-Methoden feinschleifen und dafür auch das Script von
David heranziehen als Inspiration und mein eigenes Script
perfektionieren. Eben für meine Art von Anwendung. Ich würde es auf
mehrere Files splitten und jedes File damit übersichtlicher machen.

Weiters würde ich jetzt das Computer Vision Projekt
weiterverfolgen. Dieses wurde ja erst gegen Mitte/Ende des Praktikums
aufgegriffen und eigentlich nur angerissen. Das ist ein wichtiger
Ausgangspunkt. Das freut mich sehr, dass ich nicht nur etwas
abgeschlossen habe im Rahmen des Praktikums, sondern auch gleich eine
neue Türe damit aufgestoßen habe und somit was Neues beginnen kann.
